...
<<<<<<< Updated upstream
- The post-auth dashboard is implemented as a separate `MainActivity` navigation host with a Home dashboard Fragment and independent bottom-navigation resources. Contract List/Search/Filter is available on `khai/contracts`, including a Figma-aligned filter bottom sheet for contract type, processing status, and risk level, with local visual data until a backend contract is available. Notifications, profile, and add-contract entries remain shared integration placeholders.
=======
- The post-auth dashboard is implemented as a separate `MainActivity` navigation host with a Home dashboard Fragment and independent bottom-navigation resources. Contract List/Search/Filter is available on `khai/contracts`, including a Figma-aligned filter bottom sheet for contract type, processing status, and risk level, with local visual data until a backend contract is available. Notification List is available on `khai/notifications`; it displays the Figma-aligned empty state until notification data is available, while category filters and local read-state interactions remain ready for integration. Profile is available with Figma-aligned main, edit, settings, change-password, and logout-dialog UI. Settings persists language labels and System/Light/Dark selection locally; the selected appearance applies app-wide through DayNight resources, while existing app copy remains Vietnamese. Add-contract remains a shared integration boundary.
